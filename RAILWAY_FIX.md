...
- HEALTHCHECK использует `${PORT:-5000}` для динамического порта
- Убран `EXPOSE 5000` (Railway сам инжектит PORT)
...
5. **Проверьте CORS**: Убедитесь, что `healthcheck.railway.app` разрешен
